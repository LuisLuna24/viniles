...
class Index extends Component
{
    ...
    public function estatusSubmit()
    {
        ...
        try {
            ...
            $this->reset(['statusId']);
            $this->notifications('success', 'Diseños', 'El estatus cambio con exitio');
        } catch (\Exception $e) {
            DB::rollBack();
            //dd($e->getMessage());
            $this->notifications('danger', 'Diseños', 'Lo sentimos, que ha ocurrido un error. Si el problema persiste, contacte al área de sistemas');
        }
    }
    ...
    public function deleteSubmit()
    {
        ...
        if (!Hash::check($this->password, hashedValue: Auth::user()->password)) {
            $this->notifications('danger', 'Diseños', 'La contraseña no coincide con la actual.');
            return;
        }
        ...
        try {
            ...
            $this->notifications('danger', 'Diseños', 'El dia ha sido eliminado junto a sus registros');
        } catch (\Exception $e) {
            DB::rollBack();
            //dd($e->getMessage());
            $this->notifications('danger', 'Diseños', 'Lo sentimos, que ha ocurrido un error. Si el problema persiste, contacte al área de sistemas');
